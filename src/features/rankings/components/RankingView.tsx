...
/**
 * Corpo reutilizável do ranking: pódio + lista paginada + estados de
 * loading/erro/vazio. Extraído de `GeneralRanking.tsx` (multi-championship
 * TASK-15) para ser reusado pelo Histórico (`HistoryDetail`), servindo um
 * ranking CONGELADO (snapshot `history/*`) sem qualquer regressão do ranking
 * ativo — mesmo pódio/lista/paginação, SEM `RecalcGroupRankingButton` (a ação
 * admin fica de fora deste componente; quem monta a tela decide se a inclui).
 */
 ...
 * O Histórico (TASK-15) monta esse shape a partir do snapshot congelado, com
 * `isLoading`/`isError` fixos e `refetch` no-op (sem fonte viva a reconsultar).
 */
export interface RankingViewQuery {
