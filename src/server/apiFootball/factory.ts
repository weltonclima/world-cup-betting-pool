 ...
 *
 * NOTA: a marcação `import "server-only"` NÃO fica aqui — fica no barrel
 * (./index.ts), que é o ponto de entrada público server-only. Manter este
 * módulo livre de "server-only" permite que os testes (vitest) importem o
 * factory diretamente sem que o marker (que lança fora de RSC) quebre a suíte.
 * A API pública de produção deve ser consumida via `@/server/apiFootball`.
 */
