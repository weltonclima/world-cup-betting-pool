 ...
 *
 * No App Hosting (Cloud Run) a chave vem de um secret exposto como env var.
 * A garantia server-only desta camada é dada por `import "server-only"` no
 * barrel (./index.ts) e no factory (./factory.ts).
 */
