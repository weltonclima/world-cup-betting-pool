...
Each iterable work unit carries its own status in its planning
artifact, so re-running a flow does **not** redo finished work.

| Flow | Unit | Artifact | Seeded by | Updated by |
|---|---|---|---|---|
| `/flow` | `TASK-NN` | `ai/plan/feature.md` | `/plan` | `/flow` |
| `/flow-migration` | `STEP-NN` | `ai/migration/*` | `/assess` | `/flow-migration` |
| `/flow-refactor` | `UNIT-NN` | `ai/refactor/*` | `/analyze` | `/flow-refactor` |

Two fields per unit:
- **Status** — exactly `pending` · `in-progress` · `done`. Seeded
  `pending`. A flow sets `in-progress` on start, `done` only after
  the review verdict passes (a failing review stays `in-progress`).
- **Phases done** — skills already run for the unit, appended in
  order (e.g. `spec, implement, test, review`). Lets a resumed unit
  skip completed phases, not just whole units.

Flow re-run gate: `done` → skip · `in-progress` → resume from first
phase not in `Phases done` · `pending` → run full cycle. `/progress`
reads `Status` as the authoritative signal (falls back to artifact/
git inference for older plans without the field).

`/flow-bugfix` and `/flow-hotfix` are single-pass (no generated
work-list to iterate) — no status gate applies.
