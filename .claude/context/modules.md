...
- `copaData/` — ESPN client/types/mapper/matcher + `espnBracketMap.ts` + static `teamRegistry` + `matchSource.ts` (`getEffectiveMatches` = ESPN-only + manual overlay). Legacy openfootball `client/mapper/config` (`fetchAllMatches`) is **dead code** — no consumer remains (TASK-05 done); deletion is TASK-06. See architecture.md.
