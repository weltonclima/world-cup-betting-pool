...
| `rankings` | Pool ranking, per-phase, evolution, my-rank, other-user profile. `lib/`: `rankingSort`, `accuracy`, `evolution`, `distribution`, `pagination`, `myRankingDerivations`. `hooks/`: `useRanking`, `usePoolRanking`, `useGroupRanking`, `useMyRanking`, `useParticipantProfile`, `rankingKeys`. PRD-14 adds: `lib/profilePredictionsGrouping`, `lib/bettorDna`, `lib/profileComparison`; `hooks/useProfilePredictions`; `components/profile/*` sub-components. New Route Handler `api/predictions/[uid]` (anti-cola, server-side filter `status===finished`). **PRD split-phase (entregue):** flag `pools.splitPhaseRanking: boolean` (default `false`); quando `true`, `GeneralRanking` exibe abas "Grupos" + "Eliminatórias" — dados de `pool-{poolId}-grupos` e `pool-{poolId}-eliminatorias` (já persistidos pelo recalc); `api/rankings/pool` expõe a flag no payload. Ativação: só `group_admin`/`super_admin` via `PATCH /api/group/settings`. Default `false` = comportamento cumulativo anterior. |
| `profile` | Edit profile, password, settings, history, personal stats. |
| `notifications` | List/detail + preferences. **Schema suporta 3 tipos pós-PRD-15**: `system` (moderação), `games` (jogos), `ranking`. Tipo `pool` removido em TASK-01. **V1 (pré-PRD-15)**: criação `system` client-side via `notificationFactory.ts` em `admin/lib/` — falha silenciosa para group_admin. **PRD-15 (em execução)**: `src/server/notifications/{factory,preferences,write}.ts` (server-only, Admin SDK) centraliza toda criação; triggers em Route Handlers (moderação, scoring, recalc); ID determinístico para idempotência (`games-{uid}-{matchId}`, `ranking-{uid}-{dateKey}`, `system-{uid}-{transition}-{dateKey}`); fan-out em batch chunks de 500. Preferências: `system` de moderação ignora opt-out; games/ranking respeitam. Max 50 notifs, sem paginação. Append-only (Rules: delete=false). |
...
| `worldcup` | Tournament views (groups standings, bracket) — consumes `/api/worldcup/*`. Page `/matches/eliminatorias`: `BracketView` (desktop = árvore horizontal de colunas por fase + conectores SVG reais; mobile = abas por fase com fase+próxima) + `KnockoutMatchCard` (variantes full/compact). PRD-16 (chaveamento visual) entregue. **Placar ao vivo:** `bracket.ts:deriveStatus` suporta `status="em-andamento"` (ESPN `live`) com placar parcial (`homeScore ?? 0`); KnockoutMatchCard exibe badge ao vivo. **Bug visual corrigido (TASK-04 done):** `BracketView` usa lados reais ESPN por fase (sem pareamento posicional por índice); `KnockoutMatchCard` exibe pênaltis ("1 (4)") + badge de avanço; `knockoutHelpers.ts` contém `getAdvancingSide` com gate de status e fallback por shootout para snapshots legados. `homeBracketSlot`/`awayBracketSlot`, `advanceSide`, `outcome`, `homeShootout`/`awayShootout` propagados em toda cadeia (TASK-01→04 done). **Conectores reais (TASK-07/08/09 done):** `espnBracketMap.ts` busca `matchNumber` via core API ESPN (cache 24h) e deriva `slotInRound`; `deriveBracket` em `bracket.ts` anexa `parentMatchIds` por jogo (híbrido feeder ESPN + tabela pareamento FIFA fallback). `knockoutHelpers.ts` exporta `buildMatchIndex`+`buildTreeOrder`; `BracketView` renderiza `ConnectorLayer` SVG H-bracket ancorado em `data-match-id` (ResizeObserver, aria-hidden, degrada s/ parentMatchIds). Mobile fase+próxima restaurado. CACHE_VERSION=v3 em `cache.ts` garante recompute único de snapshots stale. |
...
- `copaData/` — ESPN client/types/mapper/matcher + `espnBracketMap.ts` + static `teamRegistry` + `matchSource.ts` (`getEffectiveMatches` = ESPN-only + manual overlay). **Multi-champ (épico multi-championship-launch):** `championshipCatalog.ts` (registry estático 23 campeonatos curados: id, slug ESPN, `type: league|cup`, status, janela; helpers `getChampionship`/`isCupType`); `championshipState.ts` (status dinâmico via doc `championships/{id}` — `getChampionshipStatus`/`loadChampionshipStatuses` sobre default estático); `namespacedMatchId.ts` (`namespacedMatchId`/`parseMatchId` — `{championshipId}:{base}` p/ novos, Copa `fifa.world` emite ids legados via compat) + `matchBaseId.ts` (base=`event.id` ESPN) + `espnMatchId.ts`; `archive.ts` (arquivamento: snapshot schedule + freeze ranking/stats → `history/{poolId}/{championshipId}`, flip `status=archived`, WriteBatch atômico). `matchSource.getEffectiveMatches` escopa por `championshipId`. Legacy openfootball é **dead code**.
- `leagues/` — `standings.ts` (`computeLeagueStandings`: J/V/E/D, saldo, gols, pts; ordena pts→saldo→gols-pró→nome; gate `type==="league"`) + `standingsSource.ts`. Tabela de pontos corridos (TASK-20) — superfície de resultados p/ ligas (substitui bracket).
- `worldcup/` — `standings`, `bracket` (derived payloads) + `cache.ts` (Firestore `worldcup_cache` w/ dynamic TTL). Bracket/derivations gated a `type==="cup"` (TASK-10).
- `rankings/` — `recalc.ts` (recompute rankings/statistics/pool_stats, **escopado por campeonato**: escopos `pool-{id}-{championshipId}-{dimensão}`; scoring de liga sem bônus/best-thirds; RECALC_VERSION 5; scoring gated a `type: league` — cups não-legados geram matchId BARE que colide c/ Copa), `championshipScope.ts` (helpers de escopo por campeonato), `aggregateScopes.ts` (ranking geral agregado modo geral vs por-campeonato — TASK-12), `hydrateEntries.ts`, `avatarBudget.ts`.
