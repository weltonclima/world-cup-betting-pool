...
- **Overlay persistence (PRD-11):** `getEffectiveMatches()` (`src/server/copaData/matchSource.ts`) = openfootball base with manual overrides applied from the `matches/{id}` Firestore collection (only docs with `isManualOverride === true` win). Empty collection → identical to base. Resilient: Firestore read failure falls back to live base. Sync/edit via `/api/admin/worldcup/sync` + `/api/admin/matches/[id]` (Admin SDK only). So `matches` IS now in Firestore, but as a corrections overlay — not the primary read path.
