...
- **Coleção `predictions`**: **escrita exclusivamente via Admin SDK** (Route Handler `/api/predictions`); as Security Rules negam qualquer write do cliente. Leitura permitida apenas a usuários `approved`. `status`/`points` são gravados pelo Route Handler via Admin SDK (`src/firebase/admin.ts`), que bypassa as rules. As rules **não** lêem `kickoffAt` (partidas não estão no Firestore — arquitetura PRD-07 v2.0); o **bloqueio temporal** (`agora >= kickoffAt`) é verificado pelo **Route Handler (TASK-03)**, que rejeita a requisição antes de escrever.
